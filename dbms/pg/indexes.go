...
idx AS (
    SELECT args.db_name::text AS index_catalog,
            nr.nspname::text AS index_schema,
        ...
        WHERE i.indislive
            ...
            AND nr.nspname !~ '^pg_'
            AND ( nr.nspname = args.schema_name
                OR args.ignore_schema )
            AND ( c.relname = args.table_name
                OR args.ignore_table )
)
SELECT idx.index_catalog,
        idx.index_schema,
        idx.index_name,
        split_part ( idx.def[1], ' ', 6 ) AS index_type,
        idx.def[2] AS index_columns,
        idx.index_catalog AS table_catalog,
        idx.table_schema,
        idx.table_name,
        CASE WHEN idx.indisunique THEN 'YES' ELSE 'NO' END AS is_unique,
        idx.comments
    FROM idx
`
